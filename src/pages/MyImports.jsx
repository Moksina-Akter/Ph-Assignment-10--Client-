...
//     try {
//       const res = await fetch(`http://localhost:5000/my-imports/${id}`, {
//         method: "DELETE",
//       });
//       const data = await res.json();
//       if (res.ok) {
...
//       } else {
//         toast.error(data.message || "Failed to remove import");
//       }
//     } catch (err) {
//       console.error(err);
...
const MyImports = () => {
  const [imports, setImports] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedImport, setSelectedImport] = useState(null); // for confirm modal
      ...
      <div className="grid space-y-2 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {imports.map((imp) => (
          ...
          >
            ...
            <div className="mt-3 flex gap-2">
              <button
                onClick={() => setSelectedImport(imp)} // open confirm modal
                className="bg-red-600 text-white px-3 py-1 rounded-lg hover:bg-red-700 transition"
              >
                Remove
              </button>
